...
class indices(Resource):
    def get(self):
        return Response(pd.read_sql_table("indices_48hs",con=engine).to_json(orient="records"), mimetype='application/json')
...
class options_filtered(Resource):
...
class mep(Resource):
    def get(self,settlement_id):
        if settlement_id=="spot":
            settlement_id="ci"

        quots=pd.read_sql_table("bonds_"+settlement_id,con=engine).set_index("symbol")
...
class ALUA(Resource):
    def get(self):
        return Response(pd.read_sql_table("ALUA",con=engine).to_json(orient="records"), mimetype='application/json')


class options_strategies(Resource):
    def get(self,underlying,strategy):

        if strategy=="lanzamiento":
            return Response(lanzamiento_cubierto(underlying).to_json(orient="records"), mimetype='application/json')

        if strategy=="all":
            return Response(iv_theor_panel(underlying).to_json(orient="records"), mimetype='application/json')